...
### Fixed
- `/metrics` summed every bridge's bytes on every scrape, the same walk of the table described below, once per bridge. Prometheus scrapes every thirty seconds, so the monitoring was stopping the server for several seconds twice a minute and then reporting it as unreachable, which is where the door alarm came from. The totals are refreshed on a ten minute timer away from the request path and a scrape reads the last refresh.
- Every upload plan asked the database what the bridge had used so far, even when the bridge had no quota and no file limit to check it against. That question sums `size` across every row the bridge owns, and `size` is not in the primary key, so SQLite walks the table: on the migration bridges, six and a half million rows and four and a half seconds, run synchronously on the event loop. The server went quiet for those seconds every time a push planned its next batch, which is what the door alarm had been reporting. A bridge with no limits no longer asks.
